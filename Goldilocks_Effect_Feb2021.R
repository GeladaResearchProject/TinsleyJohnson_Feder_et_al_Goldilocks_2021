...
# Construct model
mortality.model<-glmer(Censored ~ poly(N.Females,2) + N.Males + poly(Age,2) + (1|Unit) + (1|Year), family="binomial", data=female_mortality, control=glmerControl(optimizer="bobyqa")) 
...
unit.deaths<-female_mortality %>%
  ...
  dplyr::summarise(N.Years=n(), N.Deaths=sum(Censored), Death.Rate=(N.Deaths/N.Years))
...
plot1B<-ggplot(mortality_data_summary, aes(x=as.factor(unit.cat),y=death_rate, fill=unit.cat)) + 
  geom_hline(yintercept=0.08015425, color="#5a5a5a", linetype="dotted",size=0.8)+
...
# Construct model
repro.model.6mo<-glmer(Success ~ poly(N.Females,2) + poly(Avg.Age,2) + N.Males + offset(N.Month) + (1|Unit) + (1|SuccessBin), family="binomial", data=female_repro_6mos, control=glmerControl(optimizer="bobyqa")) 
...
sum(female_repro_6mos$Success)/nrow(female_repro_6mos)

## Construct plot comparable to Plot 2D
...
plot.6mo<-ggplot(mortality_data_summary, aes(x=as.factor(unit.cat),y=success_rate*2, fill=unit.cat)) + 
  geom_hline(yintercept=0.1276297*2, color="#5a5a5a", linetype="dotted",size=0.8)+
...
plot2A<-ggplot() + scale_color_manual(values=c("#993404","#ec7014","#fec44f")) + geom_beeswarm(data = ibi_data, aes(x=unit.cat, y=IBI/365.25, color=unit.cat),size=1, cex = 2.7) + theme(legend.position="none") +
  ylab("interbirth interval (years)") + xlab("") + ylim(0.5,5.5) +
  annotate("text", x = 2, y = 5.2, label = "P=0.043", fontface=2) +
  geom_point(data=ibi_cat1, aes(x=unit.cat, y=Mean.IBI/365.25), color="black", size=2.5) +
  geom_errorbar(data=ibi_cat1, aes(x=unit.cat, ymin=min/365.25, ymax=max/365.25), color="black", width=0.15, size=1) +
  theme(plot.title = element_text(hjust = 0,face = "bold"), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(size=0.25), axis.ticks = element_line(size=0.25))
plot2A
...
plot2B<-ggplot() + scale_color_manual(values=c("#993404","#ec7014","#fec44f")) + geom_beeswarm(data = ibi_no_TO, aes(x=unit.cat, y=IBI/365.25, color=unit.cat),size=1, cex = 2.7) + theme(legend.position="none") +
  ylab("interbirth interval (years)") + xlab("unit size category") + ylim(0.5,5.5) +
  annotate("text", x = 2, y = 5.2, label = "NS", fontface=2) +
  geom_point(data=ibi_cat2, aes(x=unit.cat, y=Mean.IBI/365.25), color="black", size=2.5) +
  geom_errorbar(data=ibi_cat2, aes(x=unit.cat, ymin=min/365.25, ymax=max/365.25), color="black", width=0.15, size=1) +
  theme(plot.title = element_text(hjust = 0,face = "bold"), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(size=0.25), axis.ticks = element_line(size=0.25))
plot2B
...
survival2<-ggsurvplot(fit.unit.cat, data = infant_survival, xlim=c(0,1.5), ylim=c(.5,1), legend="none", conf.int = F,  censor=F, xlab="age (years)", ylab="proportion infants surviving", legend.labs=c("SMALL", "MEDIUM", "LARGE"), legend.title="", break.time.by=0.5, palette = c("#993404","#ec7014","#fec44f"), font.x=c(size=20), font.y=c(size=20))
survival2
...
# Construct model
mod4<-glmer(Takeovers ~ poly(N.Females,2) + N.Males + offset(N.Months) + (1|Unit)+(1|Year), family="poisson", data=unit_takeovers, control=glmerControl(optimizer="bobyqa")) 
summary(mod4)
