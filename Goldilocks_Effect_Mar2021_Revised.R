...
require(plyr);require(dplyr);require(lme4); require(lmerTest); require(MuMIn); require(ggplot2);require(ggplot2);require(ggthemes);require(ggbeeswarm);require(effects);require(coxme)
...
setwd("~/Desktop/Online Fall 2020/Papers/unit size github")
dob.ranges<-read.csv("cyphers_dobs.csv")
...
library(ggridges)
ggplot(data=female_mortality, aes(x=Age, y=unit.cat, fill=unit.cat)) + geom_density_ridges() + theme(legend.position = "none") + scale_fill_manual(values=c("#993404","#ec7014","#fec44f")) + ylab("unit size category") + xlab("age (years)")
ggsave(file="Unit_Size_FigS3.jpg", units="cm", width=24, height=18, dpi=300)
...
m2b.1 <- MCMCglmm(Censored ~ poly(N.Females,2) + Age + N.Males, random = ~Unit + Individual + Year,
                    family = "categorical", data = female_mortality, prior = prior.m2b.1,
                    verbose = FALSE)

summary(m2b.1)
plot.MCMCglmm(m2b.1)

newdat<-female_mortality
newdat$N.Males<-0
newdat$Age<-25
newdat<-newdat[,1:9]
cpred1 <- predict.MCMCglmm(m2b.1, re.form=NA, newdata=newdat, type="response", interval="confidence")

female_mortality<-cbind(female_mortality, cpred1)

ggplot(data=female_mortality, aes(y=fit,x=N.Females)) + geom_point() + geom_smooth()
...
f1 <- survfit(surv_object ~ unit.cat, data = sim.data)
f1
f.null <- survfit(surv_object ~ 1, data = sim.data)
f.null
plot(f.null)
plot(f1)

model<-coxph(surv_object ~ poly(N.Females,2) + N.Males + cluster(Unit), data = female_mortality)
summary(model)
...
plot2C 
ggsave("ProcB_Unit_Size_S2.jpg", units="cm", width=15, height=15) 
...
plot1A<-ggplot(data = x_unit,aes(x=N.Females,y=fit)) + geom_ribbon(data= x_unit, aes(x=N.Females, ymin=lower, ymax=upper, fill=unit.cat), alpha= 0.8) +
  ...
  xlab("") + ylab("deaths per female-year") + scale_x_continuous(limits=c(1,12), breaks=seq(2,12,2)) + theme(legend.position=c(0.8,0.8)) + labs(size="# female years") + theme(
...
## Make Plot 1B
mortality_data_summary<-ddply(female_mortality,. (unit.cat), summarize, ttl_fem_yrs=length(Individual), ttl_deaths = sum(Censored), death_rate = ttl_deaths/ttl_fem_yrs, stdev = sqrt(death_rate*(abs(1-death_rate))/ttl_fem_yrs), avg_age=mean(Age), stdev_age=sd(Age))
...
m2b.1 <- MCMCglmm(Success ~ poly(N.Females,2) + Age + N.Males, random = ~Unit + Individual + Year,
                  family = "categorical", data = female_repro_1mos, prior = prior.m2b.1,
                  verbose = FALSE)

summary.MCMCglmm(m2b.1)

newdat<-female_repro_1mos
newdat$N.Males<-0
newdat$Age<-10
cpred1 <- predict.MCMCglmm(m2b.1, re.form=NA, newdata=newdat, type="response", interval="confidence")

female_repro_1mos<-cbind(female_repro_1mos, cpred1)

ggplot(data=female_repro_1mos, aes(y=fit,x=N.Females)) + geom_point() + geom_smooth()

## Extract model results
effect.unit <- effects::effect(term= "poly(N.Females,2)", xlevels=10000, mod= repro.model)
x_unit<-as.data.frame(effect.unit)
x_unit$unit.cat<-"SMALL"
x_unit$unit.cat[x_unit$N.Females>4.5]<-"MEDIUM"
x_unit$unit.cat[x_unit$N.Females>7.5]<-"LARGE"
...
ggplot(data=infant.by.size, aes(x=N.Females, y=N.Year, fill=unit.cat)) +geom_bar(stat = "identity", width=1, color="black", alpha=0.8) +  scale_fill_manual(values=c("#993404","#ec7014","#fec44f")) +
  xlab("# females") + ylab("# female-years") + theme(legend.position = "none") + scale_x_continuous(breaks=seq(2,12,2), labels=seq(2,12,2)) +
  theme(plot.title = element_text(hjust = 0,face = "bold"), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(size=0.25), axis.ticks = element_line(size=0.25))
ggsave("ProcB_Unit_Size_S1.jpg", units="cm", width=20, height=15) 
...
ggsave(file="Unit_Size_Fig1.jpg", units="cm", width=24, height=24, dpi=300)
...
plot2A<-ggplot() + scale_color_manual(values=c("#993404","#ec7014","#fec44f"))+
  ...
  scale_fill_manual(values=c("#fec44f","#ec7014","#993404")) +
  annotate("text", x = 6, y = 5.2, label = "P=0.043", fontface=2) +
  theme(plot.title = element_text(hjust = 0,face = "bold"), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(size=0.25), axis.ticks = element_line(size=0.25))
plot2A
...
plot2B<-ggplot() + scale_color_manual(values=c("#993404","#ec7014","#fec44f")) + geom_point(data = ibi_no_TO, aes(x=N.Females, y=IBI/365.25),size=1.3) + theme(legend.position="none") +
  ylab("interbirth interval (years)") + xlab("") + ylim(0.5,5.5) + scale_x_continuous(limits=c(1,10),breaks=seq(2,10,2)) +
  annotate("text", x = 6, y = 5.2, label = "NS", fontface=2) + xlab("# females") + theme_minimal(base_size = 12) +
  theme(plot.title = element_text(hjust = 0,face = "bold"), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(size=0.25), axis.ticks = element_line(size=0.25))
plot2B
...
survival2<-ggsurvplot(fit.unit.cat, data = infant_survival, xlim=c(0,1.5), ylim=c(.5,1), legend="none", conf.int = F,  censor=F, xlab="age (years)", ylab="proportion infants surviving", legend.labs=c("SMALL", "MEDIUM", "LARGE"), legend.title="", break.time.by=0.5, palette = c("#fec44f","#ec7014","#993404"), font.x=c(size=20), font.y=c(size=20))
survival2
plot2C<-survival2$plot + theme(plot.title = element_text(hjust = 0,face = "bold"), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(size=0.25), axis.ticks = element_line(size=0.25))
plot2C 
...
survival.null<-ggsurvplot(fit.null, data = infant_survival, xlim=c(0,1.5), ylim=c(0,1), legend="top",  censor=F, xlab="Age (years)", ylab="Proportion infants surviving", legend.title="", break.time.by=0.5, palette = palette1)
...
ggsave(file="Unit_Size_Fig2.jpg", units="cm", width=21, height=14, dpi=300)
...
m2b.1 <- MCMCglmm(Takeovers ~ poly(N.Females,2) + N.Males + offset(N.Year), random = ~Unit + Year,
                  family = "poisson", data = unit_takeovers, prior = prior.m2b.1,
                  verbose = FALSE)
summary(m2b.1)

library(magrittr)
unit_takeovers$Predicted<-predict.MCMCglmm(m2b.1)
plot(unit_takeovers$Predicted~unit_takeovers$N.Females)

x <- unit_takeovers$N.Females
y <- unit_takeovers$Takeovers
m2b.1$Sol
mod.x <- mean(m2b.1$Sol[,1])
mod.y <- mean(m2b.1$Sol[,2])
plot(x,y)
mod.x +mod.y*x
